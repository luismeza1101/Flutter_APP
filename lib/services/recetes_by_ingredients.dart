...
class RecetesByIngredients extends StatelessWidget {
  const RecetesByIngredients({super.key});

  Future<List<Map<String, dynamic>>> fetchRecipes() async {
    const apiKey = "1452c0b80cae40c981ad2f4f8941c4c2";
    final response = await http.get(Uri.parse(
        'https://api.spoonacular.com/recipes/findByIngredients?apiKey=$apiKey&ingredients=apples,+flour,+sugar&number=5'));

    if (response.statusCode == 200) {
      final data = json.decode(response.body);

      List<Map<String, dynamic>> recipes = (data as List).map((recipe) {
        return {
